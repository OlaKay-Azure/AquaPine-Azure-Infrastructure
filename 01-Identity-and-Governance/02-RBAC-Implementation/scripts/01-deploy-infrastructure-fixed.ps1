<#
.SYNOPSIS
    Deploy AQUAPINE Azure Infrastructure Foundation (FIXED)
    
.DESCRIPTION
    Creates resource groups and storage accounts needed for RBAC testing
    Uses Azure PowerShell (Az module)
    
    FIX: Properly sets Azure context before creating storage accounts
...
Write-Host "  Azure PowerShell Implementation (FIXED)      " -ForegroundColor Cyan
Write-Host "================================================" -ForegroundColor Cyan
Write-Host ""

try {
    # Step 1: Connect to Azure and verify context
    ...
    if (-not $context) {
        throw "Failed to get Azure context. Please run Connect-AzAccount"
    }
    ...
    # Explicitly set the context (this fixes the subscription error)
    Write-Log "Setting Azure context..." -Level Info
    Set-AzContext -SubscriptionId $subscriptionId | Out-Null
    Write-Log "✅ Context set successfully" -Level Success
    Write-Host ""
    ...
    # Generate unique suffix for storage account names (lowercase alphanumeric only)
    ...
    foreach ($sa in $storageAccounts) {
        ...
        try {
            ...
            if ($existing) {
                ...
                # Create storage account with explicit parameters
                Write-Log "  → Creating storage account (this may take 30-60 seconds)..." -Level Info
            ...
            if ($_.Exception.Message -like "*subscription*") {
                Write-Log "     Subscription issue detected. Current subscription: $subscriptionId" -Level Warning
            }
        ...
        try {
            ...
            foreach ($containerName in $containers) {
                ...
                if (-not $existing) {
                    ...
                    Write-Log "  ℹ️  Container already exists: $containerName" -Level Info
                }
            }
        }
        catch {
            Write-Log "  ⚠️  Container creation warning: $($_.Exception.Message)" -Level Warning
        }
    ...
    Get-AzResourceGroup | Where-Object { 
        $_.ResourceGroupName -like "*HQ-RG" -or 
        $_.ResourceGroupName -like "*Farms-RG" -or 
        $_.ResourceGroupName -like "*Services-RG" 
    ...
    Get-AzStorageAccount | Where-Object { 
        $_.StorageAccountName -like "hrdatastorage*" -or 
        $_.StorageAccountName -like "farmmonitoring*" -or 
        $_.StorageAccountName -like "securitycctv*" 
    } | Select-Object StorageAccountName, ResourceGroupName, Location, @{
        Name='Purpose'
        Expression={$_.Tags['Purpose']}
    } | Format-Table -AutoSize
    ...
    if ($script:ResourcesFailed -eq 0) {
        ...
        if ($script:CreatedStorageAccounts.Count -eq 0) {
            Write-Host ""
            Write-Host "💡 Troubleshooting Tips:" -ForegroundColor Yellow
            Write-Host "   1. Check your Azure subscription status" -ForegroundColor Gray
            Write-Host "   2. Verify you have Contributor or Owner role" -ForegroundColor Gray
            Write-Host "   3. Try a different region if quota issues: -Location 'East US'" -ForegroundColor Gray
            Write-Host "   4. Re-authenticate: Connect-AzAccount -Force" -ForegroundColor Gray
        }
    ...
    Write-Host "1. Reconnect to Azure: Connect-AzAccount -Force" -ForegroundColor Gray
    Write-Host "2. Check subscription: Get-AzContext" -ForegroundColor Gray
    Write-Host "3. List subscriptions: Get-AzSubscription" -ForegroundColor Gray
    Write-Host "4. Set subscription: Set-AzContext -SubscriptionId 'YOUR-SUB-ID'" -ForegroundColor Gray
    Write-Host ""
    exit 1
}
